...
const defaultEnvRelPath = "scripts/staging/.env"

// LoadDefault loads scripts/staging/.env if it exists. Existing process env vars win.
func LoadDefault() (string, error) {
	path, err := findDefaultEnv()
	if err != nil {
...
func findDefaultEnv() (string, error) {
	wd, err := os.Getwd()
	if err != nil {
		return "", err
	}
	...
	for dir := wd; ; dir = filepath.Dir(dir) {
		path := filepath.Join(dir, defaultEnvRelPath)

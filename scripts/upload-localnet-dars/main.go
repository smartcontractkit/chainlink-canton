// Upload all *-1.0.0.dar packages to a local Docker Compose Canton participant.
//
...
func main() {
	...
	if len(darPaths) == 0 {
		log.Fatal("no *-1.0.0.dar files found under contracts/dars or contracts/dependencies")
	}
...
func collectDARs(repoRoot string) ([]string, error) {
	dirs := []string{
		filepath.Join(repoRoot, "contracts", "dependencies"),
		filepath.Join(repoRoot, "contracts", "dars"),
	}
	var paths []string
	for _, dir := range dirs {
		...
		for _, e := range entries {
			...
			name := e.Name()
			if strings.HasSuffix(name, "-1.0.0.dar") {

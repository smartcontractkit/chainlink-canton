...
// NOTE: This file contains the core MCMS test execution patterns.
//
// For executing MCMS proposals generated by deployment operations (with DisableMCMS=false),
// use these helper functions in combination with cantonmcms.ExecuteOutputsForTest():
//
//   - bypasserExecuteBatch() / bypasserExecuteBatchWithDisclosureParty()
//     Use for immediate execution (no timelock delay) in tests.
//
//   - scheduleExecuteOp() / executeScheduledBatchWithDisclosure()
//     Use for timelocked execution in tests.
//
//   - setRootWithRole() / setRootWithRoleAndDisclosureParty()
//     Submit signed proposals to the MCMS contract.
//
// Example flow with deployment operations:
//
//   1. Generate MCMS proposals via operations:
//      out, _ := operations.ExecuteOperation(b, fee_quoter.UpdatePrices, chain, contract.ChoiceInput[...]{})
//      // DisableMCMS defaults to false, so out.Output contains an MCMS transaction
//
//   2. Collect outputs:
...
//
//   3. Build batch:
//      batchOp, _ := cantonmcms.ExecuteOutputsForTest(outputs)
//
//   4. Execute via Bypasser (this file's helpers):
//      - Get op count: queryBypasserOpCountForParty(t, participant, party, mcmsCid)
//      - Build proposal: NewMCMSProposal(...).AddOperation(...).Build()
//      - Sign: proposal.Sign(validUntil, signers[:threshold])
//      - SetRoot: setRootWithRoleAndDisclosureParty(t, participant, owner, party, mcmsCid, "Bypasser", proposal, validUntil, signatures)
//      - Execute: bypasserExecuteBatchWithDisclosureParty(t, participant, owner, party, mcmsCid, targetCids, proposal.Operations[0], opProof)
//
// For the full SDK-based execution pattern (alternative to this file's helpers),
// see TestMCMS_ChangesetProposalE2E in changeset_mcms_test.go.

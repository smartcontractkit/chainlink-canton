...
import (
	...
	registrylnr "github.com/smartcontractkit/chainlink-canton/contracts/v2/bindings/generated/ccip/registry/lockreleasetokenpool"
...
// TestRegistryLockReleaseTokenPool_FullSendFlow proves that a registry-pools
// LockReleaseTokenPool deployed via the atomic Initialize choice (CreateAndExercise,
// no factory, no separate register step) is fully usable in a real CCIPSender.Send.
//
// Self-issued LINK is used instead of Amulet: LockReleaseTokenPool's ensure clause
// has no instrumentId.admin == poolOwner restriction (unlike BurnMintTokenPool), so
// unlike the production TestLnRTokenPool_FullSendFlow it *could* wrap a
// third-party-administered instrument. But TokenAdminRegistry.ProposeAdministrator
// only accepts a caller that is either the registry's ccipOwner or the instrument's
// own admin - Amulet's admin is a foreign Splice registry party we don't control, so
// Initialize's simple self-issued path (controller poolOwner, admin - a single actAs
// party) can't authorize it without a separate propose/accept step or cross-participant
// actAs. Using self-issued LINK (admin == poolOwner == partySender) sidesteps that
// entirely, exactly like the BurnMint variant. LinkRegistry implements
// Splice.Api.Token.TransferInstructionV1.TransferFactory in addition to
// BurnMintFactory, and its TransferFactory_Transfer takes the direct-transfer path
// whenever sender == receiver, so no TransferPreapproval is needed for this
// self-locking setup either.
//
// EDS's TokenPoolAPI does not know about the registry-pools package, so the
// pool-specific choice context and disclosures are hand-built here instead of
// fetched through EDS - mirroring exactly what
// eds/internal/api/tokenpool/tokenpool.go's lockReleaseTokenPoolSend computes for
// the production pool. Everything else (CCIP core deploy, lane config, EDS for the
// CCIP/CCV/Executor side, PerPartyRouter, CCIPSender, Send) is unchanged from
...
func TestRegistryLockReleaseTokenPool_FullSendFlow(t *testing.T) {
	...
	require.NoError(t, err)
	registryTokenPoolDar, err := contracts.GetDar(contracts.CCIPRegistryLockReleaseTokenPoolV2, contracts.DevVersion)
	require.NoError(t, err)
	...
	// Self-issued LINK is the locked/released asset - see the doc comment above for why.
	...
	// Deploy Link Token - self-issued, doubles as the pool's TransferFactory
	...
	// ------------------------------------------------------------------------
	// Deploy the registry-pools LockReleaseTokenPool via a single atomic
	// CreateAndExercise + Initialize. Self-issued LINK (admin == poolOwner ==
	// partySender) keeps Initialize's self-issued path valid - see the doc
	// comment at the top of this test for why Amulet can't be used here.
	// ------------------------------------------------------------------------
	poolInstanceID := "test-registry-pool-lnr-send"
	...
	pool := registrylnr.LockReleaseTokenPool{
		...
		TransferTimeout: registrylnr.TransferTimeout{
			RelativeHours: func(v types.INT64) *types.INT64 { return &v }(24),
		},
		Deps: registrylnr.LockReleaseTokenPoolDeps{
	...
	lane := registrylnr.LaneDeploySpec{
		...
		Inbound: registrylnr.RateLimiterDeploySpec{
			InstanceId: "test-registry-pool-lnr-send-in",
			IsEnabled:  true,
			Capacity:   "10000000000",
			Rate:       "10000000000",
		},
		Outbound: registrylnr.RateLimiterDeploySpec{
			InstanceId: "test-registry-pool-lnr-send-out",
			IsEnabled:  true,
			Capacity:   "10000000000",
			Rate:       "10000000000",
		},
		InboundCustomFinality: registrylnr.RateLimiterDeploySpec{
			InstanceId: "test-registry-pool-lnr-send-in-custom",
			IsEnabled:  false,
			Capacity:   "0",
			Rate:       "0",
		},
	...
	// instance: TokenPoolAPI doesn't know the registry-pools template, and this test
	// doesn't use TokenStandardAPI, so a pool-only instance would serve nothing.
	...
	// Mint LINK to sender - this is what gets locked into the pool.
	...
	tokenTransferHoldingCids := make([]types.CONTRACT_ID, len(senderLinkHoldings))
	for i, holding := range senderLinkHoldings {
		t.Logf("- Holding CID: %s", holding.GetCreatedEvent().GetContractId())
		tokenTransferHoldingCids[i] = types.CONTRACT_ID(holding.GetCreatedEvent().GetContractId())
	}

	// Get transfer factory for Amulet tokens (sender to CCIP owner) - fee-token leg only.
	...
	// TokenPoolAPI is hardcoded to the production lockreleasetokenpool package and
	// does not know about the registry-pools template. Mirrors what
	// eds/internal/api/tokenpool/tokenpool.go's lockReleaseTokenPoolSend computes:
	// the outbound rate limiter under "rate-limiter", the pool's own existing
	// holdings under "token-pool-holdings" (empty - nothing locked yet), and
	// LinkRegistry (which also implements TransferFactory) under "transfer-factory".
	// No transfer-preapproval entry is needed: sender == receiver == partySender
	// takes LinkRegistry's direct-transfer path, which never looks at preapproval.
	tokenPoolChoiceContext := splice_api_token_metadata_v1.ChoiceContext{
		Values: map[string]splice_api_token_metadata_v1.AnyValue{
			...
			string(registrylnr.TokenPoolHoldingsContextKey): {AVList: &[]splice_api_token_metadata_v1.AnyValue{}},
			string(registrylnr.TransferFactoryContextKey):   {AVContractId: func(v types.CONTRACT_ID) *types.CONTRACT_ID { return &v }(types.CONTRACT_ID(linkRegistryCid))},
		},
	}
	...
	// Sanity check - CCIP global batch covers P0 contracts only.
	...
	sendArgs := sender.Send{
		...
		TokenTransferInput: &sender.TokenTransferInput{
			SenderInputCids: tokenTransferHoldingCids,
	...
	// LockOrBurn (Lock path) transfers the gross amount to poolOwner directly via
	// LinkRegistry's TransferFactory (sender==receiver==partySender takes the direct
	// path); the pool fee is retained by poolOwner (== sender), so only the CCIP fee
	// leaves the sender's total Amulet balance.
	require.Eventually(t, func() bool {
	...
	// poolOwner == sender, so the locked LINK holding moves from sender to sender
	// (self-transfer): the pool holds it now, but sender's own LinkHolding balance
	// still drops by the full locked amount (senderInputCids are consumed).
	require.Eventually(t, func() bool {
		...
		return ok && linkDelta.Cmp(tokenTransferAmountRat) >= 0
	}, 15*time.Second, 200*time.Millisecond, "sender LINK holding should reflect the locked amount being consumed")

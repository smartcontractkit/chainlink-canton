...
func TestEVM2Canton_TokenLoad(t *testing.T) {
	...
	lane := devenvtests.DefaultDevenvTokenLane(t, in, lib, evmChain.ChainSelector(), cantonChain.ChainSelector())
	t.Logf("Token lane: local=%s remote=%s transfer=%s srcToken=%x",

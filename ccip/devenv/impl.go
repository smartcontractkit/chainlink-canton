...
func (c *Chain) SendMessage(ctx context.Context, dest uint64, fields cciptestinterfaces.MessageFields, opts cciptestinterfaces.MessageOptions) (cciptestinterfaces.MessageSentEvent, error) {
	...
	if ccipSendDisclosure.Executor != nil {
		...
		executorSendDisclosure, err := c.GetExecutorSendDisclosure(ctx, outgoingMessage, executorAddress, nil) // TODO CCVs
		if err != nil {
...
func (c *Chain) buildTokenTransferSendInput(
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("parse token transfer factory choice context: %w", err)
	}

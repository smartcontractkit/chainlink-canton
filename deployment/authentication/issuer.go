...
// issuerEquals reports whether a and b refer to the same authorization server issuer
// for AS metadata checks: http(s) scheme and host are compared case-insensitively, default
// ports (80 and 443) are treated as equal to the explicit :80 / :443 form, and a trailing
// "/" on the path is ignored. Userinfo, raw query, and fragment must match when present.

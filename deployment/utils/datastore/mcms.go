...
// CantonMCMSAddressRef returns the base CantonMCMS ref when present (qualifier may be empty).
func CantonMCMSAddressRef(ds datastore.DataStore, chainSelector uint64, qualifier string) (datastore.AddressRef, error) {
	filters := []datastore.FilterFunc[datastore.AddressRefKey, datastore.AddressRef]{
		datastore.AddressRefByChainSelector(chainSelector),
		datastore.AddressRefByType(datastore.ContractType(mcmsops.ContractType)),
	}
	if qualifier != "" {
	...
	case 1:
		return refs[0], nil
	case 0:
		return datastore.AddressRef{}, fmt.Errorf("no CantonMCMS ref for chain %d qualifier %q", chainSelector, qualifier)
	default:
		return datastore.AddressRef{}, fmt.Errorf("multiple CantonMCMS refs for chain %d qualifier %q", chainSelector, qualifier)
	}
}

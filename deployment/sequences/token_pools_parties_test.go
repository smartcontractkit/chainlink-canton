...
func TestResolveTokenPoolParties_requiresLabelsWithReadAs(t *testing.T) {
	t.Parallel()
...
func TestResolveTokenPoolParties_fromLabels(t *testing.T) {
	t.Parallel()

	ccip, pool, err := resolveTokenPoolParties(tokenadapters.DeployTokenPoolInput{
		TokenRef: &datastore.AddressRef{
			Labels: datastore.NewLabelSet("ccip-owner:ccip-party", "pool-owner:pool-party"),
		},
	}, cldfcanton.Participant{
		PartyID:        "operator",
		ReadAsPartyIDs: []string{"ccip-owner::abc"},
	...
	require.Equal(t, "pool-party", pool)
}

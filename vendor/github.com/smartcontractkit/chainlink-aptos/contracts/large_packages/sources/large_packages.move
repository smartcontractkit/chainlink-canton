...
module large_packages::large_packages {
    ...
    #[view]
    public fun get_code(owner: address): vector<vector<u8>> acquires StagingArea {
        let staging_area = borrow_global_mut<StagingArea>(owner);
        assemble_module_code(staging_area)
    }

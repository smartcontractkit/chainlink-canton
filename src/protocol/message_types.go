...
type TokenTransfer struct {
	...
	SourcePoolAddress        protocol.ByteSlice `json:"source_pool_address"`
	SourceTokenAddress       protocol.ByteSlice `json:"source_token_address"`
	DestTokenAddress         protocol.ByteSlice `json:"dest_token_address"`
	TokenReceiver            protocol.ByteSlice `json:"token_receiver"`
	ExtraData                protocol.ByteSlice `json:"extra_data"`
...
type Message struct {
	...
	Nonce                protocol.Nonce          `json:"nonce"`
...
func DecodeMessage(data []byte) (*Message, error) {
	...
	msg.SourceChainSelector = protocol.ChainSelector(sourceChain)
	msg.DestChainSelector = protocol.ChainSelector(destChain)
	msg.Nonce = protocol.Nonce(nonce)
...
func (m *Message) MessageID() (protocol.Bytes32, error) {
	encoded, err := m.Encode()
	if err != nil {
		return protocol.Bytes32{}, err
	}
	result := protocol.Keccak256(encoded)
	return result, nil
}
...
func (m *Message) MustMessageID() protocol.Bytes32 {
	id, err := m.MessageID()
	if err != nil {
...
func NewTokenTransfer(
	amount *big.Int,
	sourcePoolAddress, sourceTokenAddress, destTokenAddress, tokenReceiver, extraData protocol.ByteSlice,
) *TokenTransfer {
	return &TokenTransfer{
		Version:                  MessageVersion,
		Amount:                   amount,
		...
		DestTokenAddressLength:   uint8(len(destTokenAddress)),
		DestTokenAddress:         destTokenAddress,
		TokenReceiverLength:      uint8(len(tokenReceiver)),
...
func NewMessage(
	...
	onRampAddress, offRampAddress protocol.UnknownAddress,
	finality uint16,
	gasLimit uint32,
	sender, receiver protocol.UnknownAddress,
	destBlob, data []byte,
